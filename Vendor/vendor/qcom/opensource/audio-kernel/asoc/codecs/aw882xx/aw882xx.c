...
#define AW882XX_DRIVER_VERSION "v1.15.0.1"
...
//one_rst
static unsigned int one_rst_request[4] = {0};
static unsigned int one_rst_dts[4] = {0};
static unsigned int one_rst_hw[4] = {0};
	...
	struct device_node *np)
{
	...
	//one_rst
	if(one_rst_dts[aw882xx->rst_flag]==0){
	...
	if(aw882xx->rst_flag != 0)
		one_rst_dts[aw882xx->rst_flag] = 1;
	}
...
{
	int ret = 0;
	if (aw882xx->rst_flag >= (sizeof(one_rst_request) / sizeof(one_rst_request[0])))
		return -EINVAL;

	//one_rst
	if (one_rst_request[aw882xx->rst_flag] == 0) {
		if (gpio_is_valid(aw882xx->reset_gpio)) {
			...
			if (aw882xx->rst_flag != 0) {
				one_rst_request[aw882xx->rst_flag] = 1;
			}
		}
	}
		...
		struct device_node *np)
{
	...
	int32_t one_rst_flag = 0;
	
	ret = of_property_read_u32(np, "one_rst_flag", &one_rst_flag);
	if (ret) {
		aw882xx->rst_flag = 0;
		aw_dev_info(aw882xx->dev, "one_rst_flag disable!");
	} else {
		aw882xx->rst_flag = one_rst_flag;
		aw_dev_info(aw882xx->dev, "one_rst_flag enable!");
	}
...
int aw882xx_hw_reset(struct aw882xx *aw882xx)
{
	aw_dev_info(aw882xx->dev, "enter");
//one_rst
	if(one_rst_hw[aw882xx->rst_flag] == 0){
		if (gpio_is_valid(aw882xx->reset_gpio)) {
